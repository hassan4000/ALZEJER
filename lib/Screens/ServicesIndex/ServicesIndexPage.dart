...
class ServicesIndexPage extends StatefulWidget {
  @override
  _ServicesIndexPageState createState() => _ServicesIndexPageState();
}

class _ServicesIndexPageState extends State<ServicesIndexPage> {
  ...
  Future<void> getAllServicesRQ({int skip,int top,bool loadMore=false}) async {
    ...
    var response=await getAllServiceVisitor(top: top,skip: skip,filterWorkType: filterUserWork,
        filterType: DoneTxt);
    if(response.statusCode==200){
