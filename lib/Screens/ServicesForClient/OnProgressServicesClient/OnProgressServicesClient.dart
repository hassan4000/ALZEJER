...
class OnProgressServicesClient extends StatefulWidget {
  @override
  _OnProgressServicesClientState createState() => _OnProgressServicesClientState();
}

class _OnProgressServicesClientState extends State<OnProgressServicesClient> {
  ...
  Future<void> getAllServicesRQ({int skip,int top,bool loadMore=false}) async {
   ...
   if(selectedUserWork!=null &&selectedUserWork.id!=null)
    ...
    var response=await getAllServiceClient(filterUserID: userInfo.id,top: top,skip: skip,filterWorkType: filterUserWork,);
    if(response.statusCode==200){
