...
var MFSDK = new HassanFatoorah();

class HassanFatoorah implements _SDKListener {
  ...
  // Send Payment
  void sendPayment(
      String apiLang, MFSendPaymentRequest request, Function func) async {
...
//      print(result.toJson());
  ...
  void initiatePayment(
    ...
    http
      ...
      print("FATORAH COODE== "+response.statusCode.toString());
      if (statusCode < 200 || statusCode >= 400 || json == null) {
...
//        print(result.toJson());
        if (request.bypass3DS) {
          ...
          _paymentStatus(apiLang, request, func,
              cardInfoResponse: result.data, invoiceId: invoiceId.toString());
        } else {
          this.func = func;
          this.myContext = context;
...
//        print(result.data.toJson());
  ...
  void cancelToken(String token, String apiLang, Function func) async {
    ...
    http.post(uri, headers: {
      ...
      if (statusCode < 200 || statusCode >= 400 || json == null) {
        var error = MyBaseResponse.fromJson(json.decode(response.body));
        func(MFResult.fail<bool>(new MFError(statusCode, parseErrorMessage(error))));
...
//      print(result.toJson());
  ...
  void showWebView(String invoiceId, String paymentURL) async {
    Navigator.push(
      myContext,
      MaterialPageRoute(
          builder: (context) =>
              MyApp(invoiceId, paymentURL, this, appBarSpecs)),
    );
  }

  @override
  void fetchPaymentStatusByAPI(
      String invoiceId, MFPaymentStatusRequest request) {
    _paymentStatus(apiLang, request, func,
        isFinish: true, invoiceId: invoiceId);
  }
...
class MyApp extends StatefulWidget {
  ...
  MyApp(String invoiceId, String paymentURL, _SDKListener sdkListener,
      _AppBarSpecs appBarSpecs) {
  ...
  _MyAppState createState() =>
      new _MyAppState(invoiceId, paymentURL, sdkListener, appBarSpecs);
}
...
class _MyAppState extends State<MyApp> {
  ...
  _MyAppState(String invoiceId, String paymentURL,
